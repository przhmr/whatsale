...
router.get("/email-activate", async (req, res) => {
    ...
    if (token){
      ...
      jwt.verify(token, process.env.JWT_ACC_KEY, function(err,decodedToken){
        ...
        const { email, passwordHash}= decodedToken
...
exports.register = ("/register", async (req, res) => {
    try {
      let { email, password, passwordCheck, storeName } = req.body;
      ...
      if (!storeName) storeName = email;

      const salt = await bcrypt.genSalt();
      const passwordHash = await bcrypt.hash(password, salt);
      
      const token = jwt.sign({email,passwordHash}, process.env.JWT_ACC_KEY, {expiresIn:'20m'})
